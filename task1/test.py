...
parser = argparse.ArgumentParser(description="CTPN tester")
...
parser.add_argument("--")
parser.add_argument("--output-dir", action="store", help="The path to the output directory.")
...
class Evaluation:
    ...
    def __init__(self):
        path_to_images = "task1_input"
        if not osp.exists(path_to_images):
            raise ValueError("Please put image into folder " + path_to_images)
    ...
    def run(self):
        ...
        for i, image in enumerate(self.images):
            ...
            with open(osp.join(configs.OUTPUT_DIR, image_id + ".txt"), "w") as f:
